...
function generaLab(){
	...
	var cout=0;
	while(cout<laberinto.height*laberinto.width*100){
		cout++;
		var ladrillo=paredes[Math.floor(Math.random()*paredes.length)];
		...
		if (matriz[x][y].p==0&&(y%2==0||x%2==0)&&(matriz[x+direcX+direcY][y+direcY+direcX].p  +  matriz[x+direcX][y+direcY].p + matriz[x+direcX-direcY][y+direcY-direcX].p==0)&&  (matriz[x-direcX+direcY][y-direcY+direcX].p + matriz[x-direcX-direcY][y-direcY-direcX].p!==2||r==0)){
			matriz[x][y].p=1;
			paredes.push([x,y]);
		}
	}
...
function visualizarLab(){
	for (var m=0;laberinto.height>m;m++){
		for (var n=0;laberinto.width>n;n++){
			...
			ctx.fillRect(((canvas.width+ajustX)/laberinto.width)*(n-(propX-1)*(n%2)),((canvas.height+ajustY)/laberinto.height)*(m-(propY-1)*(m%2)),width-1,height-1);
			ctx.restore();
		}
	}
}
