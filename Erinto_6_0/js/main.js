...
$("#vision").change(()=>{
	...
	//scene.fog.far=vision+100;
	//scene.fog.near=vision-20;
	camera.far=vision;
...
$("#miniMapa").change(()=>{
	...
	if ($("#miniMapa").prop('checked')==true ){
		mapaON=1;
		$("#cajaMapa").css({"visibility":"visible"});
		
	}else {
		mapaON=0;
		$("#cajaMapa").css({"visibility":"hidden"});
		
	}
});
$("#retorno").click(()=>{
controls.getObject().position.set(0,10,0);
visualizarLab(controls.getObject().position.x,controls.getObject().position.z,canvas,ctx);
});
$("#butAd").click(()=>{
...
if ( havePointerLock ) {
	...
	var pointerlockchange = function ( event ) {
			if($("#run").text()=="Borrar")visualizarLab(controls.getObject().position.x,controls.getObject().position.z,canvas,ctx);
...
function animate() {
	if (mapaON==1&&$("#run").text()!="1º Generar"){
		$("#cajaMapa p").text("X: "+controls.getObject().position.x.toFixed(2) +" Y: "+(-1*controls.getObject().position.z).toFixed(2));
		visualizarLab(controls.getObject().position.x,controls.getObject().position.z,miniMapa,ctxM);
	}
