...
function setup(){
	ctx.canvas.width  = window.innerWidth/1.6;
	ctx.canvas.height = window.innerHeight/1.6;
	ctx.save();
	ctx.fillStyle='rgba(200,200,200,1)';
	ctx.fillRect(0,0,canvas.width,canvas.height);
	ctx.restore();
...
function generaLab(){
	matriz=[];
	var Puerta=Math.floor(Math.random()*((laberinto.width+laberinto.height-1)*2));

	var columna=[];

	for (var m=0;laberinto.width>m;m++){
		 columna=[];
		for (var n=0;laberinto.height>n;n++){
	...
	console.log("-Puerta-");
	var Puerta=Math.floor(Math.random()*((laberinto.width+laberinto.height-2)));
console.log("Puera: "+Puerta);
	var contador=0;
/*
	for (var m=0;laberinto.width>m;m++){
		
		for (var n=0;laberinto.height>n;n++){
			if (m==0||m==laberinto.width-1||n==0||n==laberinto.height-1&&(n%2!=0||m%2!=0)){
				contador++;
	...
	while(paredes.length>0){
		...
		for(var n=0, i =0 ;i<4;i++){
			...
			//console.log(n);
			//console.log(direccion[i][0]+" , "+direccion[i][1]);
			//console.log("dirxy: "+direccion[n]);
		...
		if (fuente==0){
				paredes.splice(randPared,1);
				//console.log("eliminado por fuente");
			}/*else {
			paredes.splice(randPared,1);
			console.log("eliminado por lonjitud");
		}*/
		
		/*
		var direcX=Math.floor(Math.random()*3)-1;
		var direcY=Math.floor(Math.random()*3)-1;
		
		var x=ladrillo[0]+direcX*(1-Math.abs(direcY));
		var y=ladrillo[1]+direcY*(1-Math.abs(direcX));
		
		if(x>laberinto.width-1)x-=2;
		else if(x<0)x+=2;
		if(y>laberinto.height-1)y-=2;
		else if(y<0)y+=2;
		...
		c=Math.floor(Math.random()*Rc);
		l=Math.floor(Math.random()*Rl);
		if (matriz[x][y].p==0&&(y%2==0||x%2==0)&&(matriz[x+direcX+direcY][y+direcY+direcX].p  +  matriz[x+direcX][y+direcY].p + matriz[x+direcX-direcY][y+direcY-direcX].p==0)&&
			(matriz[x-direcX+direcY][y-direcY+direcX].p + matriz[x-direcX-direcY][y-direcY-direcX].p!==2||r==0) &&
			(matriz[x-direcX+direcY][y-direcY+direcX].p + matriz[x-direcX-direcY][y-direcY-direcX].p!==1||c==0) &&
			(matriz[x-direcX+direcY][y-direcY+direcX].p + matriz[x-direcX-direcY][y-direcY-direcX].p!==0||l==0)){
			matriz[x][y].p=1;
			paredes.push([x,y]);
...
function visualizarLab(X,Z){
	...
	ctx.arc(x+canvas.width/2,y+canvas.height/2,30,0,(Math.PI/180)*360,true);
